...
cat("varianza totale:", varianza_totale)

# ultima ipotesi: M2V = nugget, R = range, P = smoothing parameter, V = partial sill

param_init   = c("V=0.05", "R=100", "M2V=0.04")
lower_bounds = c("V=0.01", "R=5", "M2V=0.01")
upper_bounds = c("V=0.10", "R=400", "M2V=0.08")
#equal_bounds = c("P=0.5")

sill_init = round(varianza_totale * 0.25, 4)
sill_min = round(varianza_totale * 0.01, 4)
sill_max = round(varianza_totale * 0.90, 4)
nug_init = round(varianza_totale * 0.75, 4)
nug_min = round(varianza_totale * 0.10, 4)
nug_max = round(varianza_totale * 1.20, 4)

#param_init   = c("V=0.0215", "R=100", "M2V=0.0644")
#lower_bounds = c("V=0.0009", "R=5", "M2V=0.0086")
#upper_bounds = c("V=0.0773", "R=400", "M2V=0.103")
#equal_bounds = c("P=0.5")
...
# 1. Trova i limiti dalle stazioni (con 20 km di buffer di sicurezza)
...
for (K in Ks) {
  ...
  p_check <- ggplot(check_long, aes(x = Valore)) +
    ...
    labs(
      x = "Etimated value", 
      y = "Frequency (number of grid cells)"
    ) +
    theme(
...
if(nrow(cv_results_global) > 0) {
  ...
  # 4. SALVATAGGIO
  ggsave(filename = "CV_RMSE_K_plot_pga.png", 
         plot = p_res, 
         width = 8, 
         height = 5, 
         dpi = 300, 
         bg = "white")
  saveRDS(p_res, file = "plot_cv_pga.rds")
} else {
...
dati_mappa <- res_rdd$mappa
plot_parameters(data = dati_mappa, dir_path = ".", K = "4")
