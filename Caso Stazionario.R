...
# Stime iniziali (Nugget e Sill sono uguali, Range è in METRI)
nugget_start <- 0.02
sill_start <- 0.08
range_start_metri <- 200000 # (200 km)
...
#    Partiamo dal modello fittato sui dWs (fit_exp)
...
NUGGET_STIMATO <- 0.075  # Punto di partenza sull'asse Y
SILL_PARZIALE  <- 0.10   # Quanto sale la curva (Sill Totale - Nugget = 0.175 - 0.075)
...
for(i in 1:nrow(grid_search)) {
  ...
  fit_base_ani <- vgm(psill = F_SILL, model = "Exp", range = test_range, nugget = F_NUGGET)
